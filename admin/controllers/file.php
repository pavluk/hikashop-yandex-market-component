...
class YandexMarketControllerFile extends JControllerLegacy
{
	...
	{
		if (!Factory::getUser()->authorise('core.' . strtolower($action), 'com_yandexmarket'))
		{
			// User is not authorised
			throw new Exception(Text::_('JLIB_APPLICATION_ERROR_' . strtoupper($action) . '_NOT_PERMITTED'));
		}
	...
	public function delete()
	{
		...
		{
			$this->setMessage(Text::_('COM_YANDEXMARKET_ERROR_UNABLE_TO_DELETE_FILE'), 'error');
		...
		if (!File::exists($this->folder . DS . $filename))
		{
			$this->setMessage(Text::_('COM_YANDEXMARKET_ERROR_UNABLE_TO_DELETE_FILE'), 'error');
		...
		// Удаление файла
		File::delete($this->folder . DS . $filename);
		$this->setMessage(Text::_('COM_YANDEXMARKET_DELETE_COMPLETE'));
	...
	public function create()
	{
		...
		if (empty($ymlId))
		{
			$this->setMessage(Text::_('COM_YANDEXMARKET_ERROR_UNABLE_TO_CREATE_FILE'), 'error');
		...
		{
			$ymlid = (int) $this->input->get('ymlid', 0);
			if (!empty($ymlid))
			{
				...
				$query = $db->getQuery(true)
					...
					->where('id = ' . $db->quote($ymlid))
					->where('published = 1');
				$yml   = $db->setQuery($query)->loadObject();

				if (empty($yml))
				{
					$this->setMessage(Text::_('COM_YANDEXMARKET_YML_ERROR_PUBLISHED'), 'error');

					return false;
				}
				$offers = $hikashop->getOffers(0, 0);
				...
				if ($total <= $limit)
				{
					$filename = 'turbo.xml';
					// Создаём YML-файл
					if (!$this->makeYML($folder . '/' . $filename, $offers, $ymlParams))
					{
						$this->setMessage(Text::_('COM_YANDEXMARKET_ERROR_UNABLE_TO_CREATE_FILE'), 'error');
				...
				else
				{
					...
					{
						$i++;
						if ($i <= $limit && $i < $total || ($i < $limit && $o < $total))
						{
							$includes[] = $offer;
							$o++;
						}
						$includesCount = count($includes);
						if (($includesCount == $limit) || ($total == $o))
						{
							$filename = 'turbo_' . $f . '.xml';
							if ($this->makeYML($folder . '/' . $filename, $includes, $ymlParams))
							{
								$f++;
								$includes = array();
								$i        = 0;
							...
							{
								$this->setMessage(Text::_('COM_YANDEXMARKET_ERROR_UNABLE_TO_CREATE_FILE') . ':' . $filename, 'error');
							}
						}
				...
				// Записываем в базу количество товарных предложений и дату обновления файлов
				$this->setCreatedAndOffersCount($ymlId, $total);

				$this->setMessage(Text::_('COM_YANDEXMARKET_CREATE_COMPLETE'));

				return true;
			}
			else
			{
				$this->setMessage(Text::_('COM_YANDEXMARKET_YML_ERROR_NOTFOUND'), 'error');
		...
		{
			$this->setMessage(Text::_('COM_YANDEXMARKET_HIKASHOP_NOT_INSTALLED'), 'error');
	 ...
	 * @since 0.1
	 */
	private function makeYML($pathToFile, $allOffers, $ymlParams)
	{
										...
										if (is_array($dimensions))
										{
											if (!empty($dimensions[0]))
											{
												$offerParams[$k][] = array(
													...
													"param" => array(
														"name" => Text::_("COM_YANDEXMARKET_OFFERS_PARAM_LENGTH"),
														"unit" => Text::_("COM_YANDEXMARKET_OFFERS_PARAM_SM"),
													),
												);
											}
											if (!empty($dimensions[1]))
											{
												$offerParams[$k][] = array(
													...
													"param" => array(
														"name" => Text::_("COM_YANDEXMARKET_OFFERS_PARAM_WIDTH"),
														"unit" => Text::_("COM_YANDEXMARKET_OFFERS_PARAM_SM"),
													),
												);
											}
											if (!empty($dimensions[2]))
											{
												$offerParams[$k][] = array(
													...
													"param" => array(
														"name" => Text::_("COM_YANDEXMARKET_OFFERS_PARAM_HEIGHT"),
														"unit" => Text::_("COM_YANDEXMARKET_OFFERS_PARAM_SM"),
													),
												);
											}
								...
								case 'weight':
									if (!empty($itemOffer->weight))
									{
										$offerParams[$k][] = array(
											...
											"param" => array(
												"name" => Text::_("COM_YANDEXMARKET_OFFERS_PARAM_WEIGHT"),
												"unit" => Text::_("COM_YANDEXMARKET_OFFERS_PARAM_KG"),
											),
										);
									}
									break;
